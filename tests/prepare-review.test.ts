...
async function makeReviewVault(
  ...
  const config: VaultConfig = { vault_path: vaultPath, words_folder: '' };
...
describe('prepareReview', () => {
  it('new word → bucket 1', async () => {
    ...
    try {
      ...
      expect(result.data.new_arrivals[0]).not.toHaveProperty('content');
  ...
  it('due word with sighting → bucket 2 (v2 events)', async () => {
    ...
    const { config, cleanup } = await makeReviewVault(store, sightings);
    try {
      const result = await prepareReview(config, TODAY);
      expect(result.ok).toBe(true);
      if (!result.ok) return;
      expect(result.data.used_today).toHaveLength(1);
      expect(result.data.used_today[0]).not.toHaveProperty('content');
