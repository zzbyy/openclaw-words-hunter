 ...
 */
export async function updatePage(config, input) {
    ...
    if (updated === content)
        return ok(undefined); // nothing changed
    // Write atomically
    const tmp = path.join(path.dirname(mdPath), `.wh-update-${wordLower}-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`);
    try {
