 ...
 */
export async function recordMastery(config, input) {
    // Validate inputs
    const wordErr = validateWord(input.word);
    if (wordErr)
        return { ok: false, error: wordErr };
    ...
    const bestSentences = existing?.best_sentences ?? [];
    ...
    if (escapeErr)
        return { ok: false, error: escapeErr };
    try {
        ...
        if (historyRegex.test(content)) {
            ...
            if (masteryCalloutRegex.test(content)) {
                ...
                content = content.replace(/(> \[!mastery\][\s\S]*?)(\n\n|\n##|\n---)/m, `$1\n\n### History\n${historyLine}\n$2`);
            }
            else {
                content += `\n\n### History\n${historyLine}\n`;
            }
        ...
        // Write .md atomically
        const tmp = path.join(path.dirname(mdPath), `.wh-mastery-${wordLower}-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`);
...
//# sourceMappingURL=record-mastery.js.map