 ...
 */
export async function recordSighting(config, input) {
    ...
    const tmp = path.join(path.dirname(mdPath), `.wh-sighting-${wordLower}-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`);
    try {
...
//# sourceMappingURL=record-sighting.js.map