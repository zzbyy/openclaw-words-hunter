 ...
 */
export async function fillWordPage(config, word, content) {
    ...
    if (updated.includes('{{pronunciation}}')) {
        const parts = [];
        if (content.pronunciationBrE)
            parts.push(`BrE ${content.pronunciationBrE}`);
        if (content.pronunciationAmE)
            parts.push(`AmE ${content.pronunciationAmE}`);
    ...
    if (updated.includes('{{see-also}}')) {
        const seeAlso = relatedWords.length > 0
...
// ─── Meanings block ───────────────────────────────────────────────────────────
function buildMeaningsBlock(content, lemma) {
    const allSenses = content.entries.flatMap((entry) => entry.senses.map((s) => [entry, s]));
    ...
    for (const [entry, sense] of allSenses) {
        ...
        if (sense.grammar)
            heading += ` · ${sense.grammar}`;
        if (sense.cefrLevel)
            heading += ` · ${sense.cefrLevel}`;
...
// ─── When to use block ────────────────────────────────────────────────────────
function buildWhenToUseBlock(content) {
    const seen = new Set();
    const labels = [];
...
// ─── Word family block ────────────────────────────────────────────────────────
function buildWordFamilyBlock(content) {
    if (content.wordFamily.length === 0) {
        return '*(no word family data found — add related forms manually)*\n';
    }
    return (content.wordFamily
        .map((entry) => {
        const pos = entry.partsOfSpeech.length > 0
 ...
 */
async function scanVaultForRelated(config, content, excludeWord) {
    const wordsDir = wordsFolderPath(config);
    let files;
    try {
...
//# sourceMappingURL=fill-word-page.js.map