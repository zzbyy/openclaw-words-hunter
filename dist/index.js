...
export default definePluginEntry({
    ...
    register(api) {
        ...
        const configPromise = (async () => {
            ...
            const rawVaultPath = api.pluginConfig?.['vault_path'];
            if (rawVaultPath) {
                api.logger.info(`[words-hunter] Using vault_path from plugin config: ${rawVaultPath}`);
                const bridgeResult = await loadVaultConfig(rawVaultPath);
                if (bridgeResult.ok) {
                ...
                try {
                    ...
                    const config = { vault_path: rawVaultPath, words_folder: '' };
                    await ensureConfigBridge(config);
                    await writeDiscovery(rawVaultPath, '');
                    return { ok: true, data: config };
                }
                catch {
                    return { ok: false, error: { message: `vault_path '${rawVaultPath}' does not exist on disk` } };
                }
            }
            // 3. Nothing configured
            api.logger.error('[words-hunter] Words directory not configured. Either:\n' +
            ...
            return { ok: false, error: { message: 'Words directory not configured. Install the Words Hunter macOS app, or set vault_path in plugin config.' } };
        })();
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                ...
                if (!configResult.ok)
                    return toAgentResult({ ok: false, error: { code: 'VAULT_NOT_FOUND', message: configResult.error.message } });
                const result = await scanVault(configResult.data, (params.filter ?? 'all'));
 ...
 */
async function ensureConfigBridge(config) {
...
async function fireOverdueNudges(config, logger) {
    ...
    for (const nudge of toFire) {
        if (store?.words[nudge.word]?.sessions && store.words[nudge.word].sessions > 0)
            continue;
...
async function fireWeeklyRecap(config, logger) {
    const jsonPath = masteryJsonPath(config);
    const storeResult = await readMasteryStore(jsonPath);
    if (!storeResult.ok)
        return;
...
async function persistPrimaryChannel(config, channelId) {
    const fs = await import('node:fs/promises');
    const path = await import('node:path');
    const dir = path.join(config.vault_path, '.wordshunter');
    const configPath = path.join(dir, 'config.json');
