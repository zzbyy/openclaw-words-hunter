 ...
 */
export async function onOutgoingMessage(config, messageText, channelLabel) {
    const jsonPath = masteryJsonPath(config);
    const storeResult = await readMasteryStore(jsonPath);
    if (!storeResult.ok)
        return; // no mastery data yet — nothing to match against
...
function extractSentence(text, word) {
    if (text.length <= 200)
        return null; // short enough, use as-is
    const regex = new RegExp(`[^.!?]*\\b${escapeRegex(word)}\\b[^.!?]*[.!?]?`, 'i');
    const match = regex.exec(text);
    return match ? match[0].trim() : null;
}
//# sourceMappingURL=sighting-hook.js.map